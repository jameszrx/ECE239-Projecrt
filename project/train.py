...
def main(_):
    ...
    feature_input = tf.placeholder(
        tf.float32, [None, params.LENGTH, params.FEATURE_DIMENSION], name='feature')
    ...
    for training_step in xrange(start_step, params.TRAINING_STEPS):
        training_features, training_labels = data_reader.get_data(params.BATCH_SIZE, 0, 'training')
        ...
        tf.logging.info('Step #%d: accuracy %.1f%%, cross entropy %f' %
        ...
        if (training_step % params.EVAL_STEP_INTERVAL) == 0 or is_last_step:
            ...
            for i in xrange(0, set_size, params.BATCH_SIZE):
                ...
                validation_features, validation_labels = data_reader.get_data(params.BATCH_SIZE, i, 'validation')
    ...
    for i in xrange(0, set_size, params.BATCH_SIZE):
        ...
        test_features, test_labels = data_reader.get_data(params.BATCH_SIZE, i, 'testing')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model_architecture',
        type=str,
        default='cnn_gru',  # gru, lstm, cnn_gru
